    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6 overflow-y-auto h-full">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 bg-slate-900/90 border border-white/[0.08] rounded-2xl text-xs text-slate-100 placeholder-slate-500 focus:border-indigo-500 transition-colors shadow-sm"
        />
      </div>
      ...
      {activeTab === 'all' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Targets Table */}
          <div className="lg:col-span-7 bg-slate-900/60 border border-white/[0.08] rounded-3xl overflow-hidden backdrop-blur-sm shadow-xl">
            <div className="max-h-[600px] overflow-y-auto">
              <table className="w-full text-left text-xs text-slate-300">
                <thead className="bg-slate-950/80 text-[10px] uppercase tracking-wider text-slate-400 border-b border-white/[0.06] sticky top-0">
                  <tr>
                  ...
                  {filteredTargets.slice(0, 200).map((t, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3 font-mono text-xs text-slate-400 truncate max-w-xs">
                          {t.uti}
                        </td>
                        <td className="px-5 py-3 font-medium text-slate-200 truncate max-w-xs">
                          {t.default_app?.name || (
                            <span className="text-slate-500 italic">Não configurado</span>
                          )}
                        </td>
                      </tr>
                    )
          ...
          <div className="lg:col-span-5 bg-slate-900/60 border border-white/[0.08] rounded-3xl p-6 space-y-5 backdrop-blur-sm shadow-xl">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {targetDetail.resolvedUTIs?.map((u) => (
                      <span
                        key={u}
                        className="px-2 py-0.5 rounded-lg bg-purple-950/40 border border-purple-800/40 font-mono text-[10px] text-purple-300"
                      >
                        {u}
                      </span>
                    ))}
                        ...
                        <div
                          ...
                          className={`p-2.5 rounded-xl border flex items-center justify-between text-xs transition-colors ${
                            isCurrent
                              ...
                              : 'bg-slate-950/40 border-white/[0.06] text-slate-300 hover:bg-white/[0.04]'
                          }`}
                        >
                          <div className="min-w-0 flex-1 mr-2">
                            <div className="font-semibold truncate">{appName}</div>
                            <div className="text-[10px] font-mono text-slate-400 truncate">{appPath}</div>
                          </div>
                          ...
                          {isCurrent ? (
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs transition-all disabled:opacity-50 cursor-pointer active:scale-[0.96]"
                            >
        ...
        <div className="space-y-4">
          {conflicts.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-900/60 border border-white/[0.08] rounded-3xl overflow-hidden shadow-xl">
                <table className="w-full text-left text-xs text-slate-300">
                  <thead className="bg-slate-950 text-[10px] uppercase tracking-wider text-slate-400 border-b border-white/[0.06]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-white/[0.04] font-mono">
                    {conflicts.map((c, idx) => (
                      <tr key={idx} className="hover:bg-white/[0.03]">
                        ...
                        <td className="px-6 py-3 text-xs text-slate-400 truncate max-w-xs">{c.registered_app_path}</td>
                        <td className="px-6 py-3 text-right">
                          <button
                            ...
                            className="px-3 py-1 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-sans text-xs font-semibold transition-all cursor-pointer active:scale-[0.96]"
                          >
                            Reparar / Reatribuir
                          </button>
                        </td>
